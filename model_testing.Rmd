...
vars <- summary(fullMod$finalModel)$var[1:20]
...
fullEnsPerf <- c(rmseEns, rmseGLM, rmseRF, rmseGBM, rmseFOBA, rmseXGB, rmseSVM, rmseENET)

names(fullEnsPerf) <- c("Ens", "GLM", "RF", "GBM", "FOBA", "XGB", "SVM", "ENET")
```
...
cherry_test_sub <- cherry_test %>%
  dplyr::select(c(vars, "bloom_doy", "location"))
...
subEnsPerf <- c(rmseEns, rmseGLM, rmseRF, rmseGBM, rmseFOBA, rmseXGB, rmseSVM, rmseENET)

names(subEnsPerf) <- c("Ens", "GLM", "RF", "GBM", "FOBA", "XGB", "SVM", "ENET")
