...
# Initialize matrix to store performance
ensOneYearPerf <- matrix(NA, nrow = length(evalYears), ncol = length(varNames))
...
permuteOneYearPerf <- foreach(i = 1:length(evalYears), .combine = "rbind", .packages = c("dplyr", "tidyr", "caret", "caretEnsemble")) %dopar%{
  varPerf <- c()
