...
cherry <- read.csv("data/ExCovariates.csv")
```
...
```{r, eval =F, old weather data processing}
...
fullMod <- train(x = dplyr::select(cherry_temp,-c("bloom_doy", "location")),
...
for(i in 1:ncol(cherry_test)-1){
  ...
  vars <- vars[-i]
  mod <- train(x = dplyr::select(cherry_train,-bloom_doy)[,vars],
      y = cherry_train$bloom_doy,
      distribution = "gaussian",
      method="gbm",
      metric = "RMSE",
