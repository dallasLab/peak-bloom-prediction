...
mod_list <- caretList(
  ...
  #methodList = c("gbm", "rf", "lasso", "foba", "xgbTree", "svmLinear", "enet")
)
...
rmseRidge<- sqrt(mean((compPreds[,"foba"]-cherry_test$bloom_doy)^2))
...
for(i in 1:length(evalYears)){
  ...
  ensOneYearPerf[i,] <- c(rmseEns, rmseStack, rmseRF, rmseGBM, rmseRidge, rmseLasso, rmseXGB, rmseSVM, rmseENET)
