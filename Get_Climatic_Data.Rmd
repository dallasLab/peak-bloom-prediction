---
title: "Get_Climatic_Data"
author: "Cleber Ten Caten"
date: "28/02/2022"
output: html_document
---

This script provides the code necessary to obtain the past and future climatic data for the four cities (Vacouver, Washington, Listal and Kyoto) where we have data for the cherry blooming 

```{r}
library(raster)##Necessary to handle the future data
library(rnoaa)##Necessary to download past data


```

This chunk of code is to download the historic data for the four cities
```{r}
...
##Saving the txt file that'll be used in the analyses
#setwd('peak-bloom-prediction/data')
write.table(x = historic_climate, 'data/covariates/historic_climate.txt')
```


This is code needed to manage the predicitons from future climatic data. Each of the variables used can be downloaded through python using the API request that is going to be available as comments in the following chunk of code. The zip files downloaded through the API request can then be extracted so that we can read the climatic data into R. 

```{r}
###API request for minimum temperature#####
...
setwd('/path/to/extracted_folder_min')
Arquivos<-list.files(path = "/path/to/extracted_folder_min", pattern = "_CNRM")
...
###API request for maximum temperature#####
...
setwd('/path/to/extracted_folder_max')

Arquivos.max<-list.files(path = '/path/to/extracted_folder_max', pattern = "_CNRM")
...
#   },
#   'download.zip')
setwd('/path/to/extracted_folder_prec')

Arquivos.prec<-list.files(path = "/path/to/extracted_folder_prec", pattern = "_CNRM")
...
Getting the future climatic data for each city and writing the txt file needed for the analyses.
...
write.table(x = Futclim, 'data/covariates/future_climate.txt')##precipitation might be in different measures
```
