...
```{r}
predicted_kyoto <- dplyr::filter(future_covs, year==2022 & location=="kyoto" & C_day < 45)
real_kyoto <- dplyr::filter(historic_covs, year==2022 & location=="kyoto" & C_day < 45)

predicted_vancouver <- dplyr::filter(future_covs, year==2022 & location=="vancouver" & C_day < 45)
real_vancouver <- dplyr::filter(historic_covs, year==2022 & location=="vancouver" & C_day < 45)


pdf(file="NOAAPredictsvsData.pdf")
par(mfrow=c(2,1))
plot(x=predicted_kyoto$C_day, y=predicted_kyoto$tmax, col="dark red", type="l", ylim=c(-10,15), main="Kyoto Predicted vs Observed Temps", xlab="day", ylab="Temp (C)")
points(x=real_kyoto$C_day, y=real_kyoto$tmax, col="red")
points(x=real_kyoto$C_day, y=real_kyoto$tmin, col="dark blue")
lines(x=real_kyoto$C_day, y=predicted_kyoto$tmin, col="dodger blue")

plot(x=predicted_vancouver$C_day, y=predicted_vancouver$tmax, col="dark red", type="l", ylim=c(-10,15), main="Vancouver Predicted vs Observed Temps", xlab="day", ylab="Temp (C)")
points(x=real_vancouver$C_day, y=real_vancouver$tmax, col="red")
points(x=real_vancouver$C_day, y=real_vancouver$tmin, col="dark blue")
lines(x=real_vancouver$C_day, y=predicted_vancouver$tmin, col="dodger blue")
```
...
```{r}
x <- dplyr::filter(future_covs, year==2022 & location=="kyoto" & month < 3)
table(x$location)
...
table(dplyr::filter(x, year==2022)$location)

wash <- rnoaa::ghcnd_search(stationid = "USC00186350", var = "Tmax")
...
x <- rnoaa::meteo_nearby_stations(DC_latlong, year_min = 2021, year_max=2022)


DC_1 <- rnoaa::ghcnd_search(stationid="USW00013743", date_min="2021-11-01", var="TMAX")

Lie_latlong <- data.frame("latitude"=47.4814, "longitude"=	7.730519, "id"="DC")
x <- rnoaa::meteo_nearby_stations(Lie_latlong, year_min = 2021, year_max=2022)


Lie_1 <- rnoaa::ghcnd_search(stationid="SZ000001940", date_min="2022-01-01", var="TMAX")
