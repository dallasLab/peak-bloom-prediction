...
```{r}
x <- dplyr::filter(future_covs, year==2032)
table(x$location)
```
...
x <- dplyr::filter(historic_covs, year==2022)

future_covs <- dplyr::filter(future_covs, cherry_year!=2022 | location!="kyoto" | month > 2)

z <- dplyr::filter(future_covs, cherry_year!=2022 | location!="liestal" | month > 1 | day >15)
...
```{r}
x <- dplyr::filter(historic_covs, year==2021 & location=="vancouver")
y <- dplyr::filter(historic_covs, year==2022 & location=="vancouver")
```
